<x-layouts::auth :title="__('Register')">
    <div class="flex flex-col gap-6">
        ...
        <form method="POST" action="{{ route('register.store') }}" class="flex flex-col gap-6">
            ...
            <x-input name="name" label="{{ __('Name') }}" :value="old('name')" type="text" required autofocus autocomplete="name" placeholder="{{ __('Full name') }}" inline />
            <x-input name="email" label="{{ __('Email address') }}" :value="old('email')" type="email" required autocomplete="email" placeholder="email@example.com" inline />
